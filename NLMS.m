function [f_adaptive,step_size]= NLMS(Pnum,d,u,s, f)
% s : source signal
% u : reference input
...
for num = 1
    ...
    for l = size(f,2) : M
        ...
        % w = w + mu*u*e
        p = u_block*u_block';
        n_mu = step_size_temp / (p+1);
        f_adaptive_temp = f_adaptive_temp + n_mu * e(l,1) .* u_block;
    end
...
hold on
b3 = plot(MSE_buff(:,3),'g--+'); 
hold on
b4 = plot(MSE_buff(:,4),'k--d'); 
hold off
